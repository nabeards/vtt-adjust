...
function move(cues, referenceCue, options = {}) {
  const shiftNegativeToZero = options.shiftNegativeToZero;
  ...
  var newCues = cues.map(function(_cue) {
    ...
    cue.end += delta;
    if (cue.start < 0) {
      if (shiftNegativeToZero) {
        cue.end = cue.end - cue.start;
        cue.start = 0;
      } else {
...
function moveAndScale(cues, refCue1, refCue2, options = {}) {
  const shiftNegativeToZero = options.shiftNegativeToZero;
